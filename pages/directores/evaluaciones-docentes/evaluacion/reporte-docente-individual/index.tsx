...
const ReporteDocenteIndividual = () => {
  ...
  const { buscarDocenteReporteDeEvaluacion } = UseEvaluacionDocentes()
      ...
      <button onClick={handleDownloadPdf} className='p-3 rounded-sm bg-cyan-500 text-white font-semibold w-[150px] drop-shadow-lg'>descargar pdf</button>
      {
        reporteIndividualDocente?.dni ?
          <div ref={printRef}  className='w-[1100px] p-10 bg-white'>
            <div className='mb-5'>
              <h3 className='text-3xl font-montserrat uppercase font-semibold text-slate-600'> Resultado de Evaluación de desempeño del docente</h3>
            </div>
            <div className='py-5 border-y-[1px] mb-5'>
              <h4 className='text-2xl font-montserrat uppercase text-slate-500'> Profesor: <strong>{reporteIndividualDocente.info?.nombres} {reporteIndividualDocente.info?.apellidos}</strong></h4>
