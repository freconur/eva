...
const EvaluarDocenteToEspecialista = ({ handleShowEvaluarDocente, id, getPreguntaRespuestaDocentes }: Props) => {
  ...
  const { buscarDocenteToEspecialista, guardarEvaluacionDocenteToEspecialista, resetDocente, agregarObservacionDocente } = UseEvaluacionDocentes()
  ...
  const handleSalvarPreguntaDocente = (e: React.ChangeEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (dataDocente.dni) {
      guardarEvaluacionDocenteToEspecialista(id, copyPR, dataDocente)
      setCopyPR([...originalPR])
  ...
  const handleBuscardDocente = () => {
    buscarDocenteToEspecialista(dniDocente)
  }
...
export default EvaluarDocenteToEspecialista