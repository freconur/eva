...
const EvaluarEspecialista = ({ handleShowEvaluarDocente, id, getPreguntaRespuestaDocentes }: Props) => {
  ...
  const { buscarEspecialista, guardarEvaluacionEspecialistas, resetEspecialista, agregarObservacionEspecialistas } = UseEvaluacionEspecialistas()
  ...
  const handleFinalizar = () => {
    if(dataDirector.dni) {
      agregarObservacionEspecialistas(id, dataDirector.dni, valueObservacion)
  ...
  const handleSalvarPreguntaDocente = (e: React.ChangeEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (dataDirector.dni) {
      guardarEvaluacionEspecialistas(id, copyPR, dataDirector)
      setCopyPR([...originalPR])
