...
const DeleteEvaluacionDocente = ({idEva,handleShowModalDelete}:Props) => {
  const {loaderSalvarPregunta } = useGlobalContext()
  const { deleteEvaluacion } = useAgregarEvaluaciones()
  const { deleteEvaluacionDocentes } = UseEvaluacionDocentes() 
  let container;
...
const handleDeleteEvaluacion = () => {
  deleteEvaluacionDocentes(idEva)
}
...
export default DeleteEvaluacionDocente