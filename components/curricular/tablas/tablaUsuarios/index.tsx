...
const TablaUsuarios = ({ docentesDeDirectores }: TablaUsuariosProps) => {
	...
	const { getDirectorFromEspecialistaCurricular,getNextUsuarios,getPreviousUsuarios } = useEvaluacionCurricular()
	...
	const handleNext = async () => {
		setIsLoading(true)
		try {
			const hasMore = await getNextUsuarios(lastVisible, Number(currentUserData.region))
			if (!hasMore) {
	...
	return (
		<div className={styles.tableSection}>
			<h2 className={styles.sectionTitle}>
				...
				{convertRolToTitle(currentUserData.rol || 0)}
			</h2>
